...
3. 获取 Go 源代码
	...
	从 [官方页面](https://golang.org/dl/) 或 [国内镜像](https://studygolang.com/dl) 下载 Go 的源码包到你的计算机上，然后将解压后的目录 `go` 通过命令移动到 `$GOROOT` 所指向的位置。
...
4. 构建 Go
	...
	**注意事项** 
	...
	编译1.4以上的版本会出现错误：Set $GOROOT_BOOTSTRAP to a working Go tree >= Go 1.4.，需要先编译1.4版本或下载bin版本，然后 
	增加环境变量 export GOROOT_BOOTSTRAP=$HOME/go1.4
